...
const SeatLayout = () => {
  ...
  const getShow = async () =>{
    try {
      console.log(`Fetching show data for movie ${id} on date ${date}`);
      const { data } = await axios.get(`/api/show/${id}`)
      console.log('Show API response:', data);
      if (data.success){
        // Transform the data to match expected structure
        const showsForDate = data.show.allShowTimes
          ?.filter(showTime => {
            const showDate = new Date(showTime.showDateTime).toISOString().split('T')[0];
            return showDate === date;
          })
          ?.map(showTime => ({
            time: showTime.showDateTime,
            showId: showTime._id,
            price: showTime.showPrice
          })) || [];
        
        if (showsForDate.length === 0) {
          setError(`No shows available for ${date}`);
        } else {
          const transformedShow = {
            ...data.show,
            dateTime: { [date]: showsForDate }
          };
          console.log('Transformed show data:', transformedShow);
          setShow(transformedShow);
        }
      } else {
        setError(data.message || 'Failed to fetch show data');
      }
    } catch (error) {
